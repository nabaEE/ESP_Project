...
public class LeaveTestBluestone extends BaseClass {
	static FileUtils fl = new FileUtils();
	...
	public static void cancelHalfDayODAndVerifyAttendance() {
		...
		tap.getSearchEmployeeInLeaveListByEmployee().sendKeys("EM527");
		...
		// Assert.assertTrue(ap.getAttendanceString().contains("21--D"));
	...
	public static void clickAndVerifyLeaveTypes() throws Exception {
		...
		List<String> expValues = fl.getLeaveExcelData("Bluestone", 1, 0);
		log.info("Expected leave types are :" + expValues.toString());
...
//9. Verify the leave reason mandatory
	...
	// 10. Select a date for which there is already Leave applied/approved
	@Test()
	public static void verifyErrorIfLeaveExist() throws InterruptedException {

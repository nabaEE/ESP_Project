...
public class EmployeeLeaveTestBluestone extends BaseClass {
	@Test(priority = 1)
	public static void ApplyAndApproveAdoptionLeave() throws Exception {
		log.info("---------------ApplyAndApproveAdoptionLeave:- Test Started-----------");
		...
		lp2.loginToPage(fl.getCredentials("BS", 1, 1), fl.getCredentials("BS", 1, 2));
		...
		// Call the login page class which is common for all the modules
		// Calling the method declared in login page class
		lp2.loginToPage(fl.getCredentials("BS", 2, 1), fl.getCredentials("BS", 2, 2));
		lp2.secureLogin(pObj.getProperty("USERNAME1"), pObj.getProperty("PASSWORD1"));
		WebdriverUtils.waitForPageToLoad(driver);
		driver.switchTo().frame(BaseClass.driver.findElement(By.xpath("//html//frameset//frame")));
		...
		CharSequence empName = fl.getCredentials("BS", 1, 1).subSequence(0, 6);
		...
		log.info("---------------ApplyAndApproveAdoptionLeave:- Test Ended-----------");
	}
